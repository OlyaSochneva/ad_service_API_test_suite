...
def test_create_service_card(service_card):
    print(service_card)
...
class TestFavoritesService:
    ...
    @pytest.mark.order(1)
    def test_make_service_card_favorite_success(self, user_token):
        response = requests.post(URL.SERVICE_CARDS + SERVICE_CARD_ID + "/favorite/",
    ...
    @pytest.mark.order(2)
    def test_make_service_card_favorite_twice_causes_error(self, user_token):
        response = requests.post(URL.SERVICE_CARDS + SERVICE_CARD_ID + "/favorite/",
                                 headers={"Authorization": f"Bearer {user_token}"})
        assert (response.status_code == 400 and Message.ALREADY_FAVORITE in str(response.json()))

    @allure.title('Можно удалить карточку услуг из избранного')
    @pytest.mark.order(3)
    def test_remove_service_card_from_favorites_success(self, user_token):
        response = requests.delete(URL.SERVICE_CARDS + SERVICE_CARD_ID + "/favorite/",
                                   headers={"Authorization": f"Bearer {user_token}"})
        card = requests.get(URL.CARDS + SERVICE_CARD_ID + "/")
    ...
    @pytest.mark.order(4)
    def test_remove_service_card_from_favorites_twice(self, user_token):
        response = requests.delete(URL.SERVICE_CARDS + SERVICE_CARD_ID + "/favorite/",
                                   headers={"Authorization": f"Bearer {user_token}"})
        assert (response.status_code == 400 and Message.ALREADY_NOT_FAVORITE in str(response.json()))

    @allure.title('(401)Карт. услуг нельзя добавить в избранное с отсут./невалидным токеном')
    @pytest.mark.order(5)
    ...
    def test_make_service_card_favorite_unauthorized_causes_error(self, wrong_headers, error_message):
        response = requests.post(URL.SERVICE_CARDS + SERVICE_CARD_ID + "/favorite/", headers=wrong_headers)
    ...
    def test_make_service_card_favorite_by_incorrect_id_causes_error(self, user_token, wrong_id,
    ...
    @allure.title('(404/400)Нельзя удалить из избранного карточку услуг с несуществующим/невалидным id')
    @pytest.mark.order(6)
    @pytest.mark.parametrize('wrong_id, status_code, error_message', [
        ("6666666666", 404, Message.NON_EXISTENT_CARD),
        ("pu-pu-pu", 400, Message.INVALID_ID)])
